...
async function loadAndDisplayItems(type = 'plato', initialCategory = 'todos') {
    ...
    try {
        // Hacemos una petición GET a la API, filtrando por 'tipo' (ej. 'plato')
...
document.addEventListener('DOMContentLoaded', () => {
    // Carga los ítems para el tipo 'plato' cuando la página carga
    loadAndDisplayItems('plato'); // <-- ¡IMPORTANTE! Cambia 'plato' si copias este JS para otra página de tipo diferente (bebida, licor, etc.)
    ...
    if (botonCategorias && menuCategorias) {
        ...
        document.querySelectorAll('.filtro-categorias li').forEach(item => {
            item.addEventListener('click', () => {
                if (window.innerWidth <= 600) { // Asume que 600px es el punto de quiebre para móviles
...
window.filtrar = filtrar; // La función de filtrado para los botones de categoría
// La función agregarAlCarrito ya está en carrito.js y es global